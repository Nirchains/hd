...
def get_column():
	return [_("Timesheet") + ":Link/Timesheet:120", _("Employee") + "::150", _("Employee Name") + "::150", 
		_("From Datetime") + "::140", _("Hours") + "::70", 
		_("Activity Type") + "::120", _("Task") + ":Link/Task:150", _("Subject") + "::150",  _("Description") + "::150",
...
def get_data(conditions, filters):
	time_sheet = frappe.db.sql(""" select `tabTimesheet`.name, `tabTimesheet`.employee, `tabTimesheet`.employee_name,
		`tabTimesheet Detail`.from_time, `tabTimesheet Detail`.hours,
		`tabTimesheet Detail`.activity_type, `tabTimesheet Detail`.task, `tabTimesheet Detail`.subject, `tabTimesheet Detail`.description, `tabTimesheet Detail`.project,
